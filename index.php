...
if (!isset($_GET['cat'])) { // choix du catalogue ou actions globales
  if (!isset($_GET['action'])) { // liste des catalogues pour en choisir un
    echo "<h2>POC accès aux données</h2>\n";
    echo "<h3>Utilisation du catalogue agrégé</h3><ul>\n";
    echo "<li><a href='?cat=agg&action=nbMdParOrgTheme&type=responsibleParty&arbo=arboCovadis'>",
      "Dénombrement des MDD par responsibleParty et arboCovadis</a></li>\n";
    echo "<li><a href='?action=listkws'>",
    ...
    echo "<li><a href='?cat=agg&amp;action=listkws'>Liste des mots-clés du catalogue</a></li>\n";
    echo "</ul>\n";
...
if (!isset($_GET['action'])) { // menu principal 
  ...
  echo "<li><a href='?cat=$_GET[cat]&amp;action=nbMdParTheme'>Dénombrement des MD par thème des 2 arborescences</a></li>\n";
  echo "<br>\n";
  ...
  echo "<li><a href='?cat=$_GET[cat]&amp;action=listdata'>Liste des titres de toutes les MDD (type dataset ou series)</a></li>\n";
  echo "<li><a href='?cat=$_GET[cat]&amp;action=listdataperimetre'>Liste des titres des MDD du périmètre</a></li>\n";
  echo "<li><a href='?cat=$_GET[cat]&amp;action=listdataYaml'>Liste de toutes les MDD en Yaml (type dataset ou series)</a></li>\n";
  echo "<li><a href='?cat=$_GET[cat]&amp;action=listservices'>Liste des titres des MD de service</a></li>\n";
