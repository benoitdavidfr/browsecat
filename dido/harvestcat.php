...
$pageUrl = 'https://data.statistiques.developpement-durable.gouv.fr/dido/api/harvesting/dcat-ap/id/catalog?page=1&pageSize=20';
...
$httpCache = new HttpCache('dido/pages');
...
while ($pageUrl) {
  ...
  foreach ($content['dataset'] as $datasetUrl) {
    $dataset = getRecord($httpCache, 'Dataset', $datasetUrl);
    $nbdatasets++;
    //echo Yaml::dump(['dataset'=> $dataset], 5);
  }
  $pageUrl = $content['view']['next'] ?? null;
}
echo "$nbdatasets JD détectés\n";
